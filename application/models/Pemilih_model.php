...
defined('BASEPATH') OR exit('No direct script access allowed');
  ...
  class Pemilih_model extends CI_Model{
    ...
    // Semua data pemilih
    public function allPemilih($jurusan, $perPage = 25, $offset = 0){
    ...
    public function allPemilihBelumMemilih($jurusan, $perPage = 25, $offset = 0){
    ...
    public function searchPemilih($npm, $nama, $jurusan){
      return $this->db->select('*')->from('pemilih')->join('jurusan','pemilih.id_jurusan = jurusan.id_jurusan')->where('nama_jurusan', $jurusan)->like('npm_pemilih', $nim)->like('nama_pemilih', $nama)->order_by('npm_pemilih', 'ASC')->get()->result();
    }

    public function jumlahSearchPemilih($npm, $nama, $jurusan){
      return $this->db->select('*')->from('pemilih')->join('jurusan','pemilih.id_jurusan = jurusan.id_jurusan')->where('nama_jurusan', $jurusan)->like('npm_pemilih', $nim)->like('nama_pemilih', $nama)->order_by('npm_pemilih', 'ASC')->get()->num_rows();
    }
